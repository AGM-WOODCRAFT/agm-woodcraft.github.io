...
const ServiceCard = ({ service }: { service: Service }) => (
  <div className="group relative bg-surface rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden transform hover:scale-105">
    <div
    ...
    <div className="flex flex-col items-center sm:items-start justify-center relative p-8">
      <div
        className={`inline-flex items-center justify-center w-16 h-16 rounded-xl bg-gradient-to-br ${service.gradient} text-accent mb-6 shadow-lg`}
      >
        {service.icon}
      </div>

      <h3 className="text-xl font-bold text-text mb-4 group-hover:text-text transition-colors">
        {service.title}
      </h3>

      <p className="text-muted leading-relaxed text-center sm:text-left">
        {service.description}
      </p>
    </div>
